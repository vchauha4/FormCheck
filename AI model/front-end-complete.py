...
#HAS NEW CODE IN THIS METHOD SEE INSIDE
def movenet(input_image):
    ...
    #NEW CODE -- ADJUSTS BASED ON EXERCISE WHICH DATA ANGLES ARE USED
    if(choice == 0):
...
#NEW CODE INSIDE THIS METHID BELOW
def check_reccs():
    global choice
    ...
    #SQUAT
        ...
        if(min_depth > 25):
            print("RECCOMENDATION: Youre likely not going deep enough. Try to go deeper with you're squat to maximize tension. This may require a lower weight. Lowest Depth Recorded: " + str(min_depth))
        else:
            print("OBSERVATION: You are going to optimal depth with you're squats! Lowest Depth Recorded: " + str(round(min_depth,2)))
    ...
    #CURL -- NEW CODE
        ...
        if(imbalancefoundcurl == False):
            print("OBSERVATION: No imbalance in arms found during curl!")
        else:
             print("RECCOMENDATION: Try lowering the weight as you're form is showing an imbalance")

        if(max_arm_strech > 60):
             print("OBSERVATION: You are fully extending your arm through the curl. Good job!")
        else:
            print("RECCOMENDATION: You are likely not extending your arm all the way during curl.")
...
def main(vidPath):
    ...
    while True:
        ...
        elif choice == 2: #NEW CODE HERE
            curl_recs(image,curr_kp)
...
#NEW CODE -- DIFFERENT PROCESSING CODE FOR THE CORRECT EXERCISE
'''
#Segmented each code for each of the different models
#Change this to fit however u do the backend
'''
if(choice == 0):
    print('-----Running Choice2 Bench Model-----')
    dataAngles = [[[],[]],[[],[]]]
    # print(vidpath1[84])
    ...
    print('-----Running Choice1 Squat Model-----')
    dataAngles = [[[],[]],[[],[]],[[],[]]]
    dataAngles[0][0].append(list(range(0, len(dataAngles[1][0]))))#first vid
    ...
    print("-----Running Choice2 Curl Model-----")
    OGvidPath= vidPath
...
# ---------------------FEED INTO MODEL-------------------------------------
'''
SEGMENTED THE FEEDING MODEL CODE FOR SPECIFIC EXERCISE TOO 

choice -- model name -- type

0 -- model.joblib -- Bench model
1 -- rfmodel-squat.joblib -- Varun's newest squat model --> you might have to rebuild this and add it later
2 --curl.joblib -- Curl model 
'''
...
if choice == 0:
    ...
    #rf = joblib.load("squat-model.joblib") -- older squat model

    rf = joblib.load("rfmodel-squat.joblib") #Varuns new model

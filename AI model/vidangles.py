...
EDGE_TO_COLOR = {
    ...
    (0, 5): COLORS['m'],
    (0, 6): COLORS['c'],
    (5, 7): COLORS['r'], #RIGHT SHOULDER --> Changed color to red for these
    (7, 9): COLORS['r'], #RIGHT SHOULDER
    (6, 8): COLORS['w'], #LEFT SHOULDER --> Changed color to white for these
    (8, 10): COLORS['w'], #LEFT SHOULDER
    (5, 6): COLORS['y'],
    (5, 11): COLORS['m'],
    (6, 12): COLORS['c'],
    (11, 12): COLORS['y'],
...
#Different calculate angle math taken from here --> https://prnt.sc/d3rOs4rvq7ht / https://stackoverflow.com/questions/1211212/how-to-calculate-an-angle-from-three-points
def calculate_angle_v2(p1,p2,p3):
    p1x, p1y = p1[0],p1[1]
    p2x, p2y = p2[0],p2[1]
    p3x, p3y = p3[0],p3[1]
    ...
    angleRad = np.arctan(ratio)
    joint_angle = np.abs(angleRad * 180.0 / np.pi)
...
def draw_pose(image, keypoints, radius=2):
    ...
    for c in kps:
        x, y, s = c
        if s > 0.2: #confidence is greater than 20% -- NOTE
            cv2.circle(image,
                       (int(x), int(y)),
                       radius, (41, 128, 185), -1)
...
def main(path, lpf):
    ...
    while True:
        ...
        #Draws text
        output = cv2.putText(image, str(left_shoulder_angle), (50,50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS['w'],2,cv2.LINE_AA)
        output = cv2.putText(image, str(right_shoulder_angle), (50,80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS['r'],2,cv2.LINE_AA)
...
"""
    We can add https://www.tensorflow.org/hub/tutorials/movenet to this
"""
